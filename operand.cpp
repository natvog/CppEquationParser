...
// This file - operand.cpp - contains the body of the function parseName contained in the Operand class. That function
...
Expression* Operand::parse(stringstream& in) {
   char paren;
   //int value;
   double value;       // operands can have decimal points
   
   in >> ws; 
   if (isdigit(in.peek())) {
        in >> value;
		cout << "      operand.cpp -> operand value after 'in >> value' -> " << value << endl;  //nv
